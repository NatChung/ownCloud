    ...
    if (!([self.auxUrlForReloadTable isEqualToString:@""] || ((CheckAccessToServer *)[CheckAccessToServer sharedManager]).delegate == nil)) {
        ...
        if (isCheckingTheServerRightNow) {
            ...
            if(isSSLAccepted) {
                errorImage = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"CredentialsError.png"]];
                NSString *appName = [[NSBundle mainBundle] objectForInfoDictionaryKey:@"CFBundleDisplayName"];
                label.text = (isContainWithBox101) ? [NSLocalizedString(@"server_instance_not_found",nil) stringByReplacingOccurrencesOfString:@"$appname" withString:appName] : @"Box101 server path incorrect";
                if(k_hide_url_server) {
...
-(NSString *)getUrlToCheck {
    ...
    isContainWithBox101 = [[self getUrlChecked: self.urlTextField.text].lowercaseString hasSuffix:@"box101/"];
    if( isContainWithBox101 == NO){
        url = @"";
    }
